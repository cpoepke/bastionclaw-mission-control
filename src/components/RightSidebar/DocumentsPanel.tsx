...
const DocumentsPanel: React.FC<DocumentsPanelProps> = ({
	...
	useEffect(() => {
		const load = () => {
			...
			if (selectedAgentId) query = query.eq("created_by_agent_id", selectedAgentId);
		...
		load();
		const ch = supabase
			.channel(`docs_panel_${selectedType}_${selectedAgentId ?? "all"}`)
						...
						>
							...
							<div className="flex-1 min-w-0">
								...
								<div className="text-[10px] text-muted-foreground mt-1">
									{formatRelativeTime(doc.created_at, now)}
								</div>
							</div>
							<button
								type="button"
